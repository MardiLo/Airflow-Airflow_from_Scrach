...
def source_to_use(**kwargs):
    ti = kwargs['ti']
    source = ti.xcom_pull(task_ids='hook_task')  # pull the value from hook_task
    print("source fetch from XCOM: {}".format(source))

with DAG(dag_id='xcom_dag', default_args=default_args, schedule_interval='@once', catchup=False) as dag:
